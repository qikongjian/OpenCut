...
build_frontend() {
    ...
    # 清理之前的构建
    log_info "清理之前的构建文件..."
    rm -rf .next dist node_modules/.cache apps/web/.next
...
# 准备部署文件（参考 compile_test.sh 的 dist 准备）
prepare_deployment() {
    ...
    if [ -f ".env.production" ]; then
        ...
        log_warning ".env.production 不存在，使用默认配置"
    ...
    chmod +x dist/init-database.sh
    
    log_success "部署文件准备完成" | tee -a $LOGFILE
}

# 创建部署包（参考 compile_test.sh）
create_deployment_package() {
    ...
    # 创建 tar 包（参考 compile_test.sh 的打包方式）
    tar -czvf $package_name dist/ | tee -a $LOGFILE
...
verify_deployment() {
    ...
    # 显示服务器状态
    ssh $SERVER_USER@$SERVER_HOST "cd $SERVER_PATH/current && docker-compose -f docker-compose.prod.yml logs --tail 10"
}

# 清理本地文件（参考 compile_test.sh）
cleanup() {
...
main() {
    ...
    log_info "导出API: http://$SERVER_HOST/api/export"
    log_info "健康检查: http://$SERVER_HOST/api/health"
